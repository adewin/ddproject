...
Page({
  ...
  data: {
    ...
    outTypeArray: [{ Id: 1, Name: '调出' }, { Id: 2, Name: '盘亏' }],//入库类别数组
    lydArray: [],//来源地
    categoryArray: [],//分类数组
    goodsArray: [],//品名数组
    outIndex: 0,
  ...
  getLyd() {
    ...
    dd.httpRequest({
      ...
      fail: (res) => {
        console.log("getLyd_httpRequestFail---", res)
        dd.alert({ content: JSON.stringify(res) });
      },
      complete: (res) => {
        dd.hideLoading();
 ...
 //入库类型选中事件
  ...
  submitList() {
    ...
    entityJson.OutboundType = this.data.outTypeArray[this.data.outIndex].Name;
    ...
    my.confirm({
      ...
      success: (result) => {
        ...
        if (result.confirm) {
          ...
          // }
          dd.httpRequest({
            url: domain + "/DingTalkManage/CottonYarn/SaveWarehouseOutForm",
            method: 'post',
            data: {
